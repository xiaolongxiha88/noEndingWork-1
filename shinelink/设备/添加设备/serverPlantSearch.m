...
@implementation serverPlantSearch
...
-(void)goToSearch{
    ...
    NSArray* keyArray=@[@"plantName",@"nominalPower"];
    _titleNameTypeArray=@[@"0",@"0"];
    for (int i=0; i<keyArray.count; i++) {
        NSInteger Num=[_titleNameTypeArray[i] integerValue];
        if (Num==1) {
    ...
    [_deviceNetDic setObject:@"1" forKey:@"toPageNum"];
    ...
    [BaseRequest requestWithMethodResponseStringResult:HEAD_URL paramars:_deviceNetDic paramarsSite:@"/newPlantAPI.do?op=getAllPlantList" sucessBlock:^(id content) {
        ...
        if (content1) {
            ...
            if (PlantListArray.count>0) {
                ...
                NSArray* allArray0=@[PlantListArray,netNameArray,_allNumArray];
