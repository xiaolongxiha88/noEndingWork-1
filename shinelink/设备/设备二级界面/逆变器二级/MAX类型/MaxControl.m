...
@interface MaxControl ()<UIAlertViewDelegate>
...
-(void)finishSet{
    ...
    NSDictionary *netDic=@{@"serialNum":_PvSn,@"paramId":_paramId,@"param1":_commandValue,@"param2":@""};
    
    
    [self showProgressView];
    [BaseRequest requestWithMethodResponseStringResult:HEAD_URL paramars:@{@"serialNum":_PvSn,@"paramId":_paramId,@"param1":_commandValue,@"param2":@""} paramarsSite:@"/newTcpsetAPI.do?op=maxSetApi" sucessBlock:^(id content) {
        ...
        if (content1) {
            if ([content1[@"success"] integerValue] == 0) {
                if ([content1[@"msg"] integerValue] ==501) {
                    ...
                    [self showAlertViewWithTitle:nil message:root_Device_max_495 cancelButtonTitle:root_Yes];
                }else if ([content1[@"msg"] integerValue] ==512) {
                    [self showAlertViewWithTitle:nil message:root_Device_max_496 cancelButtonTitle:root_Yes];
                }
...
-(void)control{
     ...
     UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:root_ALET message:root_NBQ_shifou_kaiqi delegate:self cancelButtonTitle:root_cancel otherButtonTitles:root_OK, nil];
    alertView.tag=4000;
    [alertView show];
...
-(void)controlOff{
    ...
    UIAlertView *alertView  = [[UIAlertView alloc] initWithTitle:root_ALET message:root_NBQ_shifou_guanbi delegate:self cancelButtonTitle:root_cancel otherButtonTitles:root_OK, nil];
        alertView.tag=4001;
    [alertView show];
}
