...
@interface LineViewForUsb () <PNChartDelegate>
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        self.unitLabel = [[UILabel alloc] initWithFrame:CGRectMake(10*NOW_SIZE, 40*HEIGHT_SIZE, 100*NOW_SIZE, 20*HEIGHT_SIZE)];
...
- (void)refreshBarChartViewWithDataDict:(NSMutableDictionary *)dataDict chartType:(NSInteger)type {
    ...
    if (_barType==3) {           //Year
          NSMutableArray *xNewArray=[NSMutableArray new];
        for (int i=0; i<_xArray.count; i++) {
            NSInteger Year=year-[_xArray[i] integerValue]+1;
            [xNewArray addObject:[NSString stringWithFormat:@"%ld",Year]];
        }
          _xArray=[NSArray arrayWithArray:xNewArray];
